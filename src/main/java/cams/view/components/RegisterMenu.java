    ...
    public RegisterMenu(Scanner scanner) {
        ...
        addItem(new ActionableItem("Attendee", new ItemAction() {
            public void execute() {
                ...
                displayController.setNextDisplay(new Alert("Successfully registered as Attendee!", new CampMenu(scanner), scanner));
            }
        }));
        ...
        addItem(new ActionableItem("Committee Member", new ItemAction() {
            public void execute() {
                if(currentUser.getCommitteeFor() != null) {
                    displayController.setNextDisplay(new Alert("Already a committee member for another camp!", new CampMenu(scanner), scanner));
                }
                else {
                    currentUser.addCamp(camp);
                    currentUser.setCommitteeFor(camp);
                    camp.addCommittee(currentUser);
                    displayController.setNextDisplay(new Alert("Successfully registered as Committee Member!", new CampMenu(scanner), scanner));
                }
            }
        }));
    }
